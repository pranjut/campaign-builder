...
trait CRUDRoute[T] extends BasicRoute[T]{
  def CRUDRoute(implicit a: akka.actor.ActorRefFactory) = new CRUDService[T].route
}

class CRUDService[T](implicit database: String,
